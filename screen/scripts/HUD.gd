...
func start_show_load_transition(fall):
	if not fall:
		...
		tween.tween_property(load_trasiton,"modulate",Color(load_trasiton.modulate.r,load_trasiton.modulate.g,load_trasiton.modulate.b,1),1)

func start_hide_load_transition():
	var tween := create_tween()
	tween.tween_property(load_trasiton,"modulate",Color(load_trasiton.modulate.r,load_trasiton.modulate.g,load_trasiton.modulate.b,0),1)
...
func _process(_delta):
	...
	exp_text.text = "EXP:{current}/{max}".format({"current": str(PlayerData.get_exp()),"max":str(PlayerData._get_next_exp())})
